...
type CashEntry = { id: string; tipo: 'ABERTURA' | 'VENDA' | 'SUPRIMENTO' | 'SANGRIA' | 'FECHAMENTO'; descricao: string; total: number; data: string };
...
function App() {
  ...
  const produtoMargemPctNumero = useMemo(
    () => (produtoCustoUnidadeNumero > 0 ? (produtoMargemReaisNumero / produtoCustoUnidadeNumero) * 100 : 0),
    [produtoCustoUnidadeNumero, produtoMargemReaisNumero]
  );
  const produtoPrecoSugeridoNumero = useMemo(
    () => (produtoCustoUnidadeNumero > 0 ? produtoCustoUnidadeNumero * 1.5 : 0),
    [produtoCustoUnidadeNumero]
  );
  ...
  const produtoMargemPctLabel = useMemo(() => `${moneyField(produtoMargemPctNumero)}%`, [produtoMargemPctNumero]);
  ...
  function handleCaixaSummaryCardClick(cardKey: KpiCardKey): void {
    ...
    if (token) void loadCaixaData(token, true);
  ...
  const cashEntriesFiltrados = useMemo(() => {
    ...
    if (!term) return cashEntries;
    return cashEntries.filter((item) => {
      const date = new Date(item.data).toLocaleString('pt-BR').toLowerCase();
      return item.tipo.toLowerCase().includes(term) || item.descricao.toLowerCase().includes(term) || date.includes(term) || String(item.total).includes(term);
    });
  }, [cashEntries, searchGrid]);
  ...
  useEffect(() => {
    ...
    const backups = loadJson<BackupData[]>(BACKUP_STORAGE_KEY, []);
    if (backups.length > 0) setLastBackupAt(backups[backups.length - 1].createdAt);
  ...
  useEffect(() => {
    if (produtoPrecoSugeridoNumero <= 0) return;
    setProdutoPreco(`R$ ${moneyField(produtoPrecoSugeridoNumero)}`);
  }, [produtoPrecoSugeridoNumero]);
  useEffect(() => {
    setHistoryPage(1);
  ...
  async function loadCaixaData(authToken: string, forceRefresh = false): Promise<boolean> {
    ...
    const [ativoPayload, historicoPayload] = await Promise.all([
      ...
      requestApi<{ dados: Array<{ id: string; tipo: CashEntry['tipo']; descricao: string | null; valor: number | string; criadoEm: string }>; total: number }>('/caixas/historico?page=1&limit=100' + suffix, {
        headers: { Authorization: `Bearer ${authToken}` },
        cache: 'no-store',
      }),
    ]);
  ...
  async function login(event: FormEvent<HTMLFormElement>): Promise<void> {
    ...
    try {
      ...
      const startTasks: Array<Promise<unknown>> = [loadProdutos(json.data.token, ''), loadVendas(json.data.token), loadCaixaData(json.data.token)];
      if (json.data.user.role === 'ADMIN') {
  ...
  function addToCart(produto: Produto): void {
    if (showPaymentModal) return;
    if (!caixaAberto) {
      setShowOpenCashModal(true);
    ...
    updateActiveCart((prev) => {
      ...
      if (!found) return [...prev, { produto, quantidade: 1 }];
      if (found.quantidade >= produto.estoque) return prev;
  ...
  async function confirmarPagamentoVenda(): Promise<void> {
    ...
    try {
      ...
      await Promise.all([loadProdutos(token, ''), loadVendas(token), loadCaixaData(token)]);
    } catch (err) {
  ...
  async function abrirCaixa(): Promise<void> {
    ...
    setError('');
    setStatus('');
    const valorInicial = moneyInput(openCashValor);
  ...
  function gerarBackupLocal(): void {
    ...
    const backups = loadJson<BackupData[]>(BACKUP_STORAGE_KEY, []);
    const nextBackups = [...backups, snapshot].slice(-30);
    saveJson(BACKUP_STORAGE_KEY, nextBackups);
    setLastBackupAt(snapshot.createdAt);
    setStatus(`Backup local salvo em ${new Date(snapshot.createdAt).toLocaleString('pt-BR')}`);
    setError('');
  }
  ...
  function restaurarUltimoBackup(): void {
    const backups = loadJson<BackupData[]>(BACKUP_STORAGE_KEY, []);
    if (backups.length === 0) {
    ...
    setProdutos(latest.data.produtos ?? []);
    setVendas(latest.data.vendas ?? []);
    setCashEntries(latest.data.cashEntries ?? []);
    setSaldoCaixa(Number(latest.data.saldoCaixa ?? 0));
    setCaixaAberto(Boolean(latest.data.caixaAberto));
    setClientes(latest.data.clientes ?? []);
    setUsuarios(latest.data.usuarios ?? []);
    setFornecedores(latest.data.fornecedores ?? []);
    setPrinterName(latest.data.printerName ?? 'Impressora térmica 80mm');
    setAutoPrint(Boolean(latest.data.autoPrint));
    setLastBackupAt(latest.createdAt);
    setStatus(`Restauração concluída a partir do backup de ${new Date(latest.createdAt).toLocaleString('pt-BR')}`);
    setError('');
  }
  ...
  async function limparHistoricoVendasApi(): Promise<void> {
    ...
    try {
      ...
      await Promise.all([loadVendas(token, true), loadCaixaData(token, true)]);
            ...
            {view === 'backup' && (
              ...
              <section className="panel config-panel">
                ...
                <div className="actions-row">
                  ...
                  <button type="button" onClick={restaurarUltimoBackup}>Restaurar backup local</button>
          ...
          <div className="cash-modal open-cash-modal">
            ...
            <div className="open-cash-fields">
              ...
              <input
                ...
                placeholder="0,00"
              />
              <small className="modal-tip">Informe valor maior que zero para abrir o caixa.</small>
            </div>

            <div className="cash-modal-actions open-cash-actions">
              <button type="button" className="secondary" onClick={() => setShowOpenCashModal(false)} disabled={openingCashLoading}>
                Cancelar
              </button>
            ...
            {produtoFormTab === 'geral' ? (
              <div className="product-editor-grid product-geral-grid">
                ...
                <div>
                  ...
                  <input value={produtoPreco} onChange={(event) => setProdutoPreco(formatCurrencyInput(event.target.value))} inputMode="numeric" />
                </div>
                <div>
                  <label>Margem de lucro (%)</label>
                  <input value={produtoMargemPctLabel} readOnly className="product-calc-input" />
                </div>
                <div>
                  <label>Estoque  *</label>
