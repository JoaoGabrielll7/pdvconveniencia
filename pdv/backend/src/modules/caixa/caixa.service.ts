import { MovimentoTipo, Prisma, Role, type FormaPagamento } from '@prisma/client';
import { prisma } from '../../config/database';
import { AppError } from '../../errors/AppError';
import type { AbrirCaixaInput, FecharCaixaInput, ListarMovimentosInput, MovimentoCaixaInput } from './caixa.schema';

type Ctx = { userId: string; userRole?: Role; ip?: string; userAgent?: string };
...
export const caixaService = {
  ...
  async historico(ctx: Ctx, query: ListarMovimentosInput) {
    const { page, limit, operadorId, dataInicio, dataFim } = query;
    ...
    if (dataInicio || dataFim) {
      ...
      if (dataInicio) criadoEm.gte = new Date(dataInicio);
      if (dataFim) criadoEm.lte = new Date(dataFim);
